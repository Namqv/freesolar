...
$(document).ready(function() {
    var e = $(".home-6 .uk-slider");
    UIkit.slider(e),
    e.on("beforeitemhide", function() {
    ...
    $(".project-zone-nav .zone-child").hasClass("active") && $(".project-zone-nav .zone-all").removeClass("active"),
    $(".product-change-view a").eq(0).trigger("click"),
    ...
    $(window).width() < 1025 && UIkit.sticky(".news-subnav", {
        offset: 40
    })
});
...
function setupImgBottomList() {
    var e = $(".img-list-wrapper .top-list").height()
      , i = $(".img-list-wrapper .top-list").width();
    $(".img-list-wrapper .bottom-list").height(e),
    $(".img-list-wrapper .bottom-list").width(i)
}
function switchImageIndex() {
    $(".img-list-wrapper .img-item a").click(function(e) {
        ...
        $(".about-2 .content-wrapper[data-index='" + i + "']").show()
    })
}
function awardSwitchSlide() {
    ...
    new Swiper($(".award-slider .swiper-container"),{
        slidesPerView: 1,
        spaceBetween: 30,
        centeredSlides: !0,
        autoplay: {
    ...
    $(".about-3 .uk-slider-items li").click(function(i) {
        i.preventDefault(),
    ...
    $(".about-3 .uk-slider-items li").eq(0).click()
}
...
function initSlider() {
    $(".square-slider").slider({
        ...
        min: 12,
        slide: function(e, i) {
            $("#square_unit").text(i.value)
            var slp=6;
            var cs=1.98;
            var sl=238;
            if(20<=i.value && i.value<=27){
                slp=10;
                cs=3.30;
                sl=396;
            }else if(28<=i.value && i.value<=35){
                slp=14;
                cs=4.62;
                sl=555;
            }else if(36<=i.value && i.value<=47){
                slp=16;
                cs=5.28;
                sl=634;
            }else if(48<= i.value && i.value<=57){
                slp=20;
                cs=6.58;
                sl=930;
            }else if(58<=i.value && i.value<=79){
                slp=26;
                cs=8.58;
                sl=1030;
            }else if(58<= i.value && i.value<=80){
                slp=32;
                cs=81.56;
                sl=1268;
            }
            if(i.value<=80)
                $("#ktdt").text('Số lượng pin có thể lắp : '+slp+ ', công suất :'+cs+' kWp, sản lượng điện trung bình tháng:'+sl+' số');
            else
                $("#ktdt").text('Dự toán hệ thống: Liên hệ tư vấn');

        },
        change: function(e, i) {
            $("#hdSquare").text(i.value),
            $("#hdSquare").val(i.value)
        }
    }),
    $(".bill-slider").slider({
        range: "min",
        ...
        step: 1e3,
        slide: function(e, i) {
            $("#bill_unit").val(numberWithDots(i.value))
        },
        change: function(e, i) {
            $("#bill_unit").val(numberWithDots(i.value))
        }
    ...
    $(".rate-slider").slider({
        ...
        step: .1,
        slide: function(e, i) {
            $("#rate_unit").val(numberWithDots(i.value))
        },
        change: function(e, i) {
            $("#rate_unit").val(numberWithDots(i.value))
        }
...
function memberSwitch() {
    $(".member-tabs .member-content").eq(0).addClass("show"),
    $(".member-slider .item").eq(0).addClass("active"),
    $(".member-slider .item").click(function(e) {
        ...
        $(".member-tabs .member-content[data-index='" + i + "']").addClass("show")
    })
}
function changeContactMapSrc() {
    $(".address-slider .item").click(function() {
        ...
        $(this).addClass("active");
        var e = $(this).data("src");
        $("#contactMap").attr("src", e)
    }),
    $(".address-slider .item").eq(0).trigger("click")
}
    ...
    $("body").on("click", "a.ajaxpagerlink", function(e) {
        ...
        if (t.length)
            return $.ajax({
                ...
                success: function(e) {
                    $(".ajaxresponse .ajaxresponsewrap").append($(e).find(".ajaxresponsewrap").html()),
                    $(".ajaxresponse .ajaxresponsewrap").after($(e).find(".faq-view-more")),
                    !1,
                    i.parent().remove()
                }
            }),
...
function addClassHeaderWhenScroll() {
    $(window).on("scroll", function() {
        $("header").toggleClass("minimal", $(window).scrollTop() > 0)
    })
}
function changeViewProduct() {
    $(".product-change-view a").click(function(e) {
        ...
        $(".product-view[data-view='" + i + "']").addClass("show")
    })
}
function setWidthLineInSlider() {
    if ($(".solution-6").length > 0) {
        var e = $(".process-slider .icon").eq(0).offset().left
          , i = $(".process-slider .swiper-container").width();
        $(".process-slider .line").css({
            width: i - 30 - e / 2
        })
    }
    if ($(".development-4").length > 0) {
        var t = $(".development-4 .icon").eq(0).offset().left
          , s = $(".development-4 .swiper-container").width();
